      ...
      <div className="mt-10 flex flex-col gap-3">
        {
          objs.map((obj,i) => (
            <div key={i}>
              <div className="flex gap-6 p-0 items-center ">
                <h2 className="m-0 ">{obj.package}</h2>
                <p className="m-0">{obj.date}</p>
              </div>
              <div className="text-secondary">ETB {obj.price}</div>
              <div>
                <p className="m-0 p-0">{obj.from} - {obj.to}</p>
                <p className="m-0 p-0">{obj.type}</p>
                {obj.calls} calls
              </div>
              <div className="flex">
                Payment Status
                <div className={'flex-1 text-center '+(obj.status === 'Completed'?"text-green-500":'text-secondary')}>{obj.status}</div>
              </div>
