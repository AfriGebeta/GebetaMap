        ...
        (type) => (
          <div>
            <Link to={to} onClick={handleClick} className={"flex gap-1  items-center mt-2 text-white hover:!text-[#ffa81d] relative w_hvr "+(active?'!text-secondary':'')}>
              ...
              <span className={"inline-block absolute sm:relative flex-1 p-0 left-12 sm:left-0 hvr_trgt sm:hvr_trgtx shadow-xl shadow-black sm:shadow-none whitespace-nowrap bg-[#1e1e1e] sm:bg-transparent "}>{label}</span>
              {children && (type !== 'hidden') ?
                dropped ? (
                  <span className="-mt-1 px-2 mx-2 hidden sm:flex justify-end w-5 border-transparent border overflow-hidden"><RightOutlined className="!font-extrabold" /></span>
                ):(
                  <span className="mt-1 px-2 hidden sm:flex items-end h-3 border-transparent border overflow-hidden "><DownOutlined className="!font-extrabold  inline-block" /></span>
                )
              : null}
            </Link>
    ...
    <div className="!bg-dark ">
      <div className="flex flex-col ">
        <div className="flex items-center text-white text-child border-b border-gray-700 shadow-md py-2 ">
          ...
          <div className="flex items-center relative ml-10">
            <div className="flex gap-4  ">
              ...
              <div className={"absolute right-5 top-[118%] shadow-lg border border-[#333] shadow-black p-4 bg-[#202022] !z-50 w-[400px] max-w-[90vw] h-[300px] "+notifyModal}>
                <Notification />
              </div>
        ...
        <div className="flex relative ">
          {/* SideBar */}
          <div className={"h-full min-h-screen flex flex-col pr-2 sticky  top-0 shadow-xl shadow-black z-10 bg-dark py-5 "+type}>
            ...
            <SideLink label='Sign Out' iconComp={LogoutOutlined} to="/v2/" onClick={()=>{dispatch(setUser({}))}} />
