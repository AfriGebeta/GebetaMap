...
function OnDemand() {
  const handleTssData = (e, index, type) => {
    const newTssData = [...tssData];
    const ds = {
      longitude:
        type != "longitude" ? newTssData[index].longitude : e.target.value,
      latitude:
        type != "latitude" ? newTssData[index].latitude : e.target.value,
      placename:
        type != "placename" ? newTssData[index].placename : e.target.value,
    };
    newTssData[index] = ds;
    setTssData(newTssData);
  };

  const getPlacename = (index) => {
    return tssData[index].placename;
  };

  const getLatitude = (index) => {
    return tssData[index].latitude;
  };

  const getLongitude = (index) => {
    return tssData[index].longitude;
  };

  const [manualLocArr, setManualLocArr] = useState([
    <ManualLoc
      index={0}
      handleData={handleTssData}
      handleGetLongitude={getLongitude}
      handleGetLatitude={getLatitude}
      handleGetPlaceName={getPlacename}
    />,
  ]);
          ...
          <div className="flex gap-4 items-start flex-wrap">
            <div className="flex flex-1 gap-4 items-start flex-wrap">
              <div className="flex flex-col flex-1 gap-4">
                {manualLocArr.map((ManualElem, i) => ManualElem)}
              </div>
              <input
                type="submit"
                value={count >= 9 ? "Calculate" : "+"}
                onClick={addLocation}
          ...
          <div className="flex gap-4 flex-wrap">
            <div className="leading-3 flex-1  flex gap-3 p-4 rounded-xl bg-[#202022] ">
              ...
              <input
                type="submit"
                value="PDF"
                className="btn_sty1 !bg-btnprimary !border-btnprimary"
              />
              <input
                type="submit"
                value="CSV"
                className="btn_sty1 !bg-[#181818] !border-[#181818]"
              />
              <input
                type="submit"
                value="JSON"
