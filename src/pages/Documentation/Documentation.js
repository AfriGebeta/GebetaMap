...
const customDot = (dot, { status, index }) => (
  <Popover>
    <span
      className={
        "w-full h-full inline-block rounded-full m-0 p-0 " +
...
function ScreenChange({ children, visible, index, setCurrentView }) {
  useEffect(() => {
    if (visible) setCurrentView(index);
  }, [visible]);
  return children;
}
          ...
          <div className="p-6 bg-[#202022] text-[#777] rounded-xl">
            ...
            <div className="leading-3 py-3">
              {userData.token ? (
                <Link
                  to="/v2/account"
                  className="btn_sty1 float-right bg-secondary/40 hover:text-secondary"
                >
                  Home
                </Link>
              ) : (
            ...
            >
              ...
              >
                {items.map((item) => (
                  <Step
                    title={item.title}
                    className="!text-[#aaa] text-child force"
                  />
                ))}
              </Steps>
            </ConfigProvider>
        ...
        <div className="flex-1 flex flex-col gap-4 p-10 card rounded-md bg-[#202022]">
          <div>
            ...
            <div className="py-3 flex gap-4 items-center">
              <Link to="/#" className="btn_sty1 sml bg-secondary/40">
                Routing API
              </Link>
              <Link to="/#" className="btn_sty1 sml ">
                Matrix API
              </Link>
              <Link to="/#" className="btn_sty1 sml ">
